...
export function computeDossierScore(
  ...
  if (lspSeverity === "VETO" || lspState === "TAKEOVER") {
    lspPoints = -25;
  } else if (lspSeverity === "REJECT" || lspState === "HOSTILE") {
    lspPoints = -15;
  ...
  if (combo && combo.rankingDelta !== 0) {
    factors.push({
      ...
      detail: `Market × Contract × Regime combination ${combo.state} (${combo.sampleSize} trades)`,
    });
  ...
  if (cal) {
    let calPoints = 0;
    if (
      cal.reliabilityState === "RELIABLE" &&
      cal.calibratedProbability >= (cal.theoreticalBaseline ?? 0.5)
    ) {
      calPoints = 3.0;
    } else if (cal.reliabilityState === "UNDERPERFORMING") {
      calPoints = -6.0;
    }
    if (calPoints !== 0) {
      factors.push({
