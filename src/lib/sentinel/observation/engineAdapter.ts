...
export function mapIntelToObservationInputs(
  ...
  return intel.contracts.map((c: any): EngineEvidenceInput => {
    ...
    if (lspState === "TAKEOVER" && lsp.verdict === "VETO") {
      lspSeverity = "VETO";
    ...
    } else if (regimeReport.isChanging || intel.regime?.label === "CHOPPY") {
    ...
    const vetoRes = resolveVeto(
      `${marketId}:${prop}`,
      {
        ...
        losingSideVeto: lspState === "TAKEOVER" && lsp.verdict === "VETO",
    ...
    const comboResult = comboLearning.lookup({
      ...
      contract: c.id,
      regime: intel.regime?.label ?? "UNKNOWN",
      entryCondition: entryLabRec?.entryConditionId ?? "IMMEDIATE",
    });
    ...
    const governedSpine = runVetoEngine(contractSpine, governance);
    const operatorSpecial = operatorSpecialDigitAction(
      intel.symbol,
      c.id,
      digits as number[],
      canonicalState.red,
      canonicalState.secondRed,
      canonicalState.green,
      canonicalState.secondGreen,
    );
    ...
    return {
      ...
      momentum: {
        ...
        raw: { spinePressure: spineReport.pressure, wsm },
      },
      trigger: {
        state: triggerState,
        raw: { entryLabRec },
