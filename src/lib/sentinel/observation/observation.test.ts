...
describe("Sentinel Observation Layer — §20 Consolidated Master Test Suite", () => {
  ...
  it("32. reaches RIPE with realistic noisy tick stream (36 supporting out of 40 ticks = 90% support)", () => {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      input.entryDigit = {
        digit: 4,
        state: "VALIDATED",
        support: isNoise ? "NEUTRAL" : "SUPPORTING",
        dangerousCompetitor: false,
      };
      input.pressure.byWindow = {
        15: "SUPPORTING",
  ...
  it("34. correctly transitions DECAYING to EXPIRED when elapsed ticks exceed EXPIRE_OBSERVATION_SAMPLES", () => {
    ...
    for (let i = 0; i < 130; i++) {
      ...
      inactive.psychology = { direction: "NONE", state: "DORMANT", support: "UNKNOWN" };
  ...
  it("49. qualityBand on RankedOpportunity matches assessQuality() for the same dossier across quality bands", () => {
    ...
    exceptionalInput.momentum = {
      ...
      winningSideMomentum: {
        state: "SURGING",
        surgeFactor: 2.0,
        consecutiveSurgeSamples: 3,
        momentumVelocity: "RAPID",
        persistenceScore: 80,
        dominanceScore: 85,
        entrySafetyDiscount: 0,
        raw: null,
      },
    };
    ...
    strongInput.momentum = {
      ...
      winningSideMomentum: {
        state: "BUILDING",
        surgeFactor: 1.5,
        consecutiveSurgeSamples: 2,
        momentumVelocity: "STEADY",
        persistenceScore: 70,
        dominanceScore: 75,
        entrySafetyDiscount: 0,
        raw: null,
  ...
  it("39. rankOpportunities produces honest unvalidated defaults with zero fabricated convergence/combination/direction scores", () => {
    ...
    expect(opp!.combination.wins).toBe(0);
    expect(opp!.combination.n).toBe(0);
    expect(opp!.combination.state).not.toBe("VALIDATED");
