      ...
      measurability: intel.regime
        ? clamp(
            (intel.regime.confidence ??
              (intel.regime.strength ? intel.regime.strength * 100 : 70)) / 100,
            0.3,
            1,
          )
        : 0,
      detail: intel.regime
        ? `${intel.regime.label} (confidence ${(intel.regime.confidence ?? (intel.regime.strength ? intel.regime.strength * 100 : 70)).toFixed(0)}). ${c.regimeNote || (c.regimeCompatible ? "Regime supports this contract." : "Regime is outside this contract's validated range.")}`
