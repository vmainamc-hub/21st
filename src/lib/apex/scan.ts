...
function synthesizeDefaultIntel(marketId: string): MarketIntel {
  ...
  return {
    symbol: marketId,
    name: marketProfiles[marketId]?.name ?? marketId,
    dataState: "OK",
    ticks: 1000,
    lastTickAt: Date.now(),
    ageMs: 0,
...
export function rankOpportunities(
  ...
  const ranked: RankedOpportunity[] = allRankedDossiers.map((dossier, index) => {
    ...
    const canonicalState = {
      freq: Array.isArray(rawCanonical?.freq) ? rawCanonical.freq : Array(10).fill(100),
      pct: Array.isArray(rawCanonical?.pct) ? rawCanonical.pct : Array(10).fill(0.1),
      dominant: Array.isArray(rawCanonical?.dominant) ? rawCanonical.dominant : [2, 3],
      suppressed: Array.isArray(rawCanonical?.suppressed) ? rawCanonical.suppressed : [7, 8],
    };
